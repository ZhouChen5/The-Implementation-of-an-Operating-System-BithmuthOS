...
pub static mut CLIPBOARD: [char; 80] = ['\0'; 80];
pub static mut CLIPBOARD_LEN: usize = 0;
...
impl Shell {
    ...
    fn interpret(&mut self) {
        match self.buffer {
            ...
            _b if self.is_command("uname") => unsafe {
                lib::println!("BismuthOS v1.0 IA-32 x86");
            },
            _b if self.is_command("whoami") => unsafe {
                lib::println!("root");
            },
            _b if self.is_command("mouse") => unsafe {
                let (x, y) = crate::drivers::mouse::MOUSE.get_position();
                lib::println!("Mouse position: x={}, y={}", x, y);
            },
            _b if self.is_command("pickchar") => unsafe {
                match crate::drivers::mouse::MOUSE.get_last_pick() {
                    Some((row, col, ch)) => {
                        lib::println!("Last picked char at (row={}, col={}): '{}' (0x{:02X})", row, col, ch as char, ch);
                    },
                    None => {
                        lib::println!("No character picked yet.");
                    }
                }
            },
            b if b[0] == '\0' || b[0] == '\n' => {}
    ...
    pub fn set_clipboard(&mut self, text: &[char]) {
        unsafe {
