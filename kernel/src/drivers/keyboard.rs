    ...
    unsafe {
        ...
        if E0_PREFIX {
            ...
            match scancode {
                ...
                // 检测Ctrl+Shift+C
                0x2E => { // 'C'键
                    let ctrl = KEYBOARD.left_ctrl || KEYBOARD.right_ctrl;
                    let shift = KEYBOARD.left_shift || KEYBOARD.right_shift;
                    if ctrl && shift {
                        if let Some(buf) = MOUSE.get_selection_text() {
                            let len = buf.iter().position(|&c| c == '\0').unwrap_or(buf.len());
                            SHELL.set_clipboard(&buf[..len]);
                        }
                        return;
                    }
                }
