...
build:
	...
	@cargo build --target=x86_16-bismuth.json --package=boot
	@cargo build --target=x86_16-bismuth.json --package=bootloader
	@cargo build --target=x86_32-bismuth.json --package=kernel
	@echo "[INFO] Building applications..."
	$(foreach app,$(APPS),cargo build --target=x86_32-bismuth.json --package=$(app);)

.PHONY: objcopy
objcopy:
	@echo "[INFO] Copying ELF binaries to binary format..."
	@mkdir -p build/apps
	@$(OBJCOPY) -I elf32-i386 -O binary target/x86_16-bismuth/debug/boot build/boot.bin
	@$(OBJCOPY) -I elf32-i386 -O binary target/x86_16-bismuth/debug/bootloader build/bootloader.bin
	@$(OBJCOPY) -I elf32-i386 -O binary target/x86_32-bismuth/debug/kernel build/kernel.bin
	$(foreach app,$(APPS),$(OBJCOPY) -I elf32-i386 -O binary target/x86_32-bismuth/debug/$(app) build/apps/$(app).bin;)
